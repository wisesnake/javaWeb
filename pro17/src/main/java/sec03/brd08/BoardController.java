	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (action == null || action.equals("/listArticles.do")) {
				...
				//최초 접속시에는 첫 페이지일테니 section, pagenum을 1로 초기화
				//페이지네이션의 페이지숫자 버튼을 누르면 해당 숫자가 그대로 섹션과 페이지넘으로 넘어옴.
				Map pagingMap=new HashMap();
				pagingMap.put("section", section);
				pagingMap.put("pageNum", pageNum);
				Map articlesMap=boardService.listArticles(pagingMap);
				//섹션과 페이지넘버를 매핑해서 페이지 게시판 데이터 가져오는 메소드 호출
				//쿼리문이 section과 pagenNum을 이용해서 쿼리문을 조작하여 10개단위로
				//가져오는 방식으로 이루어져 있으므로 페이지에 맞는 데이터를 가져오게 됨.
				articlesMap.put("section", section);
				articlesMap.put("pageNum", pageNum);
				request.setAttribute("articlesMap", articlesMap);
				//articlesMap에는 페이지별 게시판 글들과, section,pageNum이 매핑되어있음.
				...
				// 키,값을 문자열을 제너릭으로 사용하는 map 컬렉션 데이터 생성 후,
